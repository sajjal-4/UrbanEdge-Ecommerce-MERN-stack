...
async function initItems() {
    ...
    try {
      ...
      products.forEach(({ image, name, price, _id, category }) => {
        ...
        let detailsLink = '';
        if (category === 'shoes') {
          detailsLink = '/shoesdetails';
        } else if (category === 'clothing') {
          detailsLink = '/clothesdetails';
        } else if (category === 'jewelry') {
          detailsLink = '/jewellerydetails';
        }
        ...
        card.innerHTML = `
          ...
          <div class="card-body">
              ...
              <button class="add-to-cart btn btn-dark form-control" onclick="addToCart('${_id}')">Add to Cart</button>
              <a href="${detailsLink}?id=${_id}" class="btn btn-info form-control mt-2">View Details</a>
          </div>
      `;
      ...
      itemList.innerHTML = '<p>Error loading products.</p>';
    }
  }
  ...
  async function initDeals() {
    try {
      ...
      const dealIdToShowcaseMap = {
        '6798c207497c836d31ff3872': '[data-deal-id="6798c207497c836d31ff3872"]',
        '6798c207497c836d31ff3873': '[data-deal-id="6798c207497c836d31ff3873"]',
        '6798c207497c836d31ff3874': '[data-deal-id="6798c207497c836d31ff3874"]'
      };
  
      deals.forEach((deal) => {
        const showcaseSelector = dealIdToShowcaseMap[deal._id];
        const showcaseElement = document.querySelector(showcaseSelector);
  
        if (showcaseElement) {
          updateShowcaseElement(showcaseElement, deal);
        } else {
          console.error(`Showcase element not found for selector: ${showcaseSelector}`);
        }
      });
    } catch (error) {
      console.error('Error:', error);
  ...
  function updateShowcaseElement(showcaseElement, deal) {
    // Update image
    const imgElement = showcaseElement.querySelector('.showcase-img');
    if (imgElement) {
      imgElement.src = deal.image;
      imgElement.alt = deal.name;
    }
  
    // Update title
    const titleElement = showcaseElement.querySelector('.showcase-title a');
    if (titleElement) {
      titleElement.textContent = deal.name;
    }
  
    // Update description
    const descElement = showcaseElement.querySelector('.showcase-desc');
    if (descElement) {
      descElement.textContent = deal.description;
    }
  
    // Update price
    const priceElement = showcaseElement.querySelector('.price');
    if (priceElement) {
      priceElement.innerHTML = `<b>$${deal.price.toFixed(2)}</b>`;
    }
    const discountElement = showcaseElement.querySelector('.price-box > span');
    if (discountElement) {
      discountElement.style.display = deal.discount ? 'inline' : 'none';
      discountElement.textContent = deal.discount ? deal.discount : '';
    }
    if (addToCartBtn) {
      // Remove the previous onclick attribute
      addToCartBtn.removeAttribute('onclick'); 
  
      // Add the new onclick attribute with the correct deal ID
      addToCartBtn.setAttribute('onclick', `addToCart('${deal._id}')`);
    }
  }
  
  
  // Add to Cart Function (updated to use MongoDB)
